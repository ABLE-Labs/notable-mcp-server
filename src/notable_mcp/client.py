...
class RobotClient(Protocol):
    """Common interface for NotableClient and SimulatedClient."""
    ...
    async def drop_tip(self, pipette_number: int, deck_number: int | None = None, well: str | None = None) -> dict: ...
    async def move_to(self, pipette_number: int, deck_number: int, well: str, z_reference: str = "top_just", z_speed: int | bool = True) -> dict: ...
    async def move_z(self, pipette_number: int, deck_number: int, z_reference: str = "bottom", z_speed: int | bool = True) -> dict: ...
    async def aspirate(self, pipette_number: int, volume: float, flow_rate: float | bool = True) -> dict: ...
    async def dispense(self, pipette_number: int, volume: float, flow_rate: float | bool = True) -> dict: ...
    async def blow_out(self, pipette_number: int) -> dict: ...
    async def ready_plunger(self, pipette_number: int) -> dict: ...
    async def mix(self, pipette_number: int, cycle: int, volume: float, flow_rate: float | bool = True, delay: float = 0) -> dict: ...
    # Action
    ...
    async def shaker_shake(self, rpm: int, duration_sec: float, accel_sec: int = 5) -> dict: ...
